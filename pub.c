...
//也可以使用inet_ntoa(struct in_addr in)
...
void socket_accept(int st)
{
	...
	while (1)//循环执行accept
	{
		...
		client_st = accept(st, (struct sockaddr *)&client_addr, &len);//从套接口st接收到连接，放入client_addr

		if (client_st == -1)
		{
			printf("accept 接收失败 %s\n", strerror(errno));
			...
			*tmp = client_st;
			{
				...
				pthread_create(&thr_d, &attr, socket_contr, tmp);//注意tmp要手动在堆上分配因为要传递到另一个域
			}
		}
	}
}
